...
void Reach::compute_basic_coefficients() {
    ...
    for(size_t i = 1; i < this->nSec; i++){
        ...
        sa = fabs(fdSec->u) * 9.81 * pow(fdSec->roughness, 2) * dx / (pow(fdSec->r, 4.0/3) * 2 * this->dev_sita);//TODO using sec roughness
        sb = fabs(bdSec->u) * 9.81 * pow(bdSec->roughness, 2) * dx / (pow(bdSec->r, 4.0/3) * 2 * this->dev_sita);
